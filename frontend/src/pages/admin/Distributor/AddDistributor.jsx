...
const AddDistributor = () => {
  ...
  const [licenseApplications, setLicenseApplications] = useState([createEmptyDistributionApplication()]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await distributorApi.createDistributor(createDistributorPayload);

      const distributorId = response?.data?.data?.user?.id;
      const firstApplicationId = response?.data?.data?.distributionApplication?.id;

      if (!distributorId) {
        throw new Error('Distributor ID not found after creation');
      }

      if (!firstApplicationId) {
        throw new Error('Distribution application ID not found after creation');
      }
      ...
      for (let index = 0; index < additionalApplications.length; index += 1) {
        ...
        const additionalResponse = await distributorApi.createDistributorApplication(distributorId, buildApplicationPayload(application));
      ...
      toast.success(`Distributor created with ${licenseApplications.length} license application(s) successfully`);
      navigate('/admin/distributors');
    } catch (error) {
      console.error('Failed to create distributor:', error);
      const errorMsg = error.response?.data?.message || 'Failed to create distributor. Please try again.';
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Create distributor account and distribution license application details"
        actions={
