...
const AdminLicenses = () => {
  ...
  const fetchApplications = useCallback(async () => {
    ...
    try {
      ...
      const distribution = (distributionRes.data?.data || []).map((item) => ({
        ...
        applicantName: item.applicantName,
        applicantEmail: item.applicantEmail,
        businessAddress: item.warehouseAddress,
        district: item.district,
        province: item.province,
        submittedById: item.submittedBy?.id || null
      }));
  ...
  const resolveHolderPath = (type, holderId) => {
    ...
    if (type === 'MANUFACTURING') return `/admin/manufacturers/${holderId}`;
    if (type === 'DISTRIBUTION') return `/admin/distributors/${holderId}`;
    return `/admin/retailers/${holderId}`;
  };
