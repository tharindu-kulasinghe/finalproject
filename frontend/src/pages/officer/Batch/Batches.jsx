...
const OfficerBatches = () => {
  ...
  const handleReview = async () => {
    ...
    try {
      ...
      console.error('Failed to review batch:', err);
  ...
  const columns = [
  {
    key: 'batchNo',
    header: 'Batch No',
    render: (row) => <span className="font-mono text-xs font-semibold text-gray-900">{row.batchNo}</span>
  },
  {
    key: 'product',
    header: 'Product',
    ...
    <div>
          ...
          <p className="text-xs text-gray-500">{row.license?.companyName || ''}</p>
        </div>
  ...
  },
  {
    key: 'output',
    header: 'Output',
    render: (row) => <span>{formatNumber(row.outputLiters)} L</span>
  },
  {
    key: 'units',
    header: 'Units',
    render: (row) => formatNumber(row.unitsProduced)
  },
  {
    key: 'status',
    header: 'Status',
  ...
  },
  {
    ...
    <div className="flex items-center gap-1.5">
          <Button size="sm" variant="outline" onClick={() => navigate(`/officer/batches/${row.id}`)}>View</Button>
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Verify daily production updates submitted by manufacturers" />
      ...
      <div className="border border-gray-200 bg-white">
        <div className="p-4 border-b border-gray-100 flex flex-col lg:flex-row gap-3">
          <SearchBar
            value={search}
            onChange={setSearch}
            placeholder="Search by batch no, product, or manufacturer..."
            className="lg:w-80" />
        ...
        <Table columns={columns} data={filtered} loading={loading} emptyMessage="No batches found" />
      </div>
